...
**`EventTool` owns event publication and completion; its `task_finished` event records the result and may create a child task. `FinishTool` is the automatically registered wrapper, with a bare-object shorthand for ordinary results.**

`event({ "name": "task_finished", "data": { ... } })` routes `data` through the same completion engine as `finish({ ... })`: it records the result through `Queue::set_result`, then transitions the task to `Finished`. Before that dispatch, `FinishTool` wraps a non-empty object carrying none of its reserved arguments as `{ "result": object }`; its established envelope passes through unchanged. Every other event name is observational, even when it is a built-in lifecycle name. `Queue::emit_event` remains observational too. The loop still enforces `finish` on every non-interactive agent because `FinishTool` remains the default and `EventTool` is opt-in.
...
- The loop binds the task schema into both `FinishTool` forms and, when registered, `EventTool.data.result`. `FinishTool` uses `if`/`then`/`else` so only the selected bare or envelope branch repairs values. A result that misses its schema is rejected before completion, and one written as JSON text inside `result` is decoded there. Their one own check: a handover needs a real result.
...
- `result`, `handover`, and `task` select `finish`'s explicit envelope. A result object declaring any of those properties therefore uses `finish({ "result": object })`; every other non-empty result object may be passed bare. The same three fields always sit inside `event.data` for `task_finished`.
