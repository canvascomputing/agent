...
- Handlers MUST be cheap and non-blocking; the loop does not await them. `on_result_async` is the one exception, and the loop still never awaits: registering one only queues the finished ticket, and whichever `finish` is waiting drains it and awaits each handler on its own task. A handler that never returns therefore stalls the caller rather than an agent, and a `start()`-only host uses `on_result`.
...
- `ModelRequest.tools` carries the `Tool` values themselves rather than a second description of them, and the registry they come from is the ticket's own: `claim` clones the agent's registry and registers `FinishTool::from_schema(ticket.schema)` over it. A provider never calls a tool's handler.
