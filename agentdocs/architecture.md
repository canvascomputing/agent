...
- The tool runtime races every public custom-handler future against the run ending and drops the handler when cancellation wins. Dropping the `Queue` while agents still hold a `Weak` is the public way to abort: the upgrade fails and each task panics out cleanly.
