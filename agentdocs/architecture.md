...
- The queue is the first parameter of every handler. That is what let the `create_ticket_on_*` hooks go: a handler files its own follow-up work through `queue.ticket(..)` and selects what it needs through `queue.find_*`, without an `Arc` into the queue that holds it.
