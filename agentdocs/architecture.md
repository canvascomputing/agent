...
- `Stats::stats_for_label(label)` returns a nested `Stats` slice scoped to one label, and `stats_for_agent(agent_name)` does the same per agent. `stats_by_label()` and `stats_by_agent()` list them. Reading a slice does not create one: `record_scoped` and the store use the `pub(crate)` `slice_for_label` and `slice_for_agent` for that, so a misspelled lookup leaves nothing behind in `stats.json`.
...
- Two measures stay run-wide by design. `run_duration()` is `None` on a slice, since elapsed run duration is global, and `usage_for_ticket()` is empty there, since it is the compaction estimator's per-ticket series rather than a figure about the run.
