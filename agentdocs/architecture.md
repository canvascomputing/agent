...
- Failures are the plural mirror of the single result. `TicketQueue::emit` pushes every failure event onto `Ticket::errors`, so a ticket accumulates the failed requests and tool calls it saw. A failure is not a transition: an entry lands whether or not the ticket goes on to fail, so a `Finished` ticket can carry some. Nothing is written for it: the line is already in `<dir>/events.jsonl`, and `TicketQueue::load` folds the failures back onto each ticket in the one pass it makes over the log for `Stats`. `is_recorded_failure` is the single definition both paths read (`EventKind::is_failure()` bar the `TicketFailed` marker, which `status` and `failed_at` already carry), so a live ticket and a resumed one carry the same entries. There is no per-ticket errors file and no second writer to keep in step.
...
**`TicketQueue::emit` folds every event into the crate-private `Stats` and appends it to `events.jsonl`, `TextChunkReceived` aside. A host reads the log back through `TicketQueue::find_events`; the crate counts only what a limit check needs.**
