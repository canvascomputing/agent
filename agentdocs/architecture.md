...
- Every registered tool carries a schema, so no call reaches a tool unchecked. A tool whose own document does not compile is held to `{"type": "object"}` instead: the author's mistake must not cost the agent both the check and a callable tool.
