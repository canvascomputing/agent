...
- `Ticket::schema(...)` attaches a `Schema` to the ticket; the finisher tool validates the result and the loop applies `max_schema_retries` on mismatch. `handover_ticket` accepts any JSON `result` and validates it against the parent ticket's own schema, like `finish_ticket`. A schema mismatch in `handover_ticket` aborts before the child is inserted, so neither the parent's finish nor the child happens: the operation stays atomic.
