Read and write tasks in the Werk. One `action` per call.
...
- ALWAYS finish your current task with `finish` before the response ends, or it stays `in_progress` and the loop re-picks it.
