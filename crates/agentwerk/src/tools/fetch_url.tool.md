...
- Authenticated or private URLs fail; do not retry, fall back to a specialized tool or `bash` with credentials.

## When NOT to use

- Private/authenticated URLs (Nextcloud, GitLab, Confluence, Jira, dashboards): check for a specialized tool first.
- GitHub URLs: prefer `gh pr view` / `gh issue view` / `gh api` via `bash`.
- Custom headers, methods, or bodies: use `bash` with `curl`.
