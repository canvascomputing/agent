//! The reply as it arrives: the content blocks a stream grows one fragment at a
//! time, and the [`ModelResponse`] they assemble into.
//!
//! A provider decodes its own payloads and names which call each one is. Which
//! block a fragment continues, and when a [`StreamEvent`] fires, are decided
//! here, so the two vendors cannot disagree about either.
...
/// Read an SSE reply to its end through `decode`, and give back what it built.
pub(crate) async fn assemble(
    ...
    stream::for_each_data(response, |json| decode(json, &mut reply)).await?;
    reply.into_response()
}

/// Which tool call a fragment continues. An endpoint that numbers its calls
/// names one; for the rest the builder assigns one, and the two can never
/// collide.
...
/// The reply a stream is building.
...
impl ResponseBuilder {
    ...
    /// Report that the model said the request outgrew its context window. The
    /// reply is then a failed request rather than an answer, so
    /// [`into_response`](Self::into_response) gives back the error the loop
    /// summarizes on.
    ...
    pub(crate) fn add_thinking(&mut self, fragment: &str) {
        ...
        match self.blocks.last_mut() {
            Some(Block::Thinking { thinking, .. }) => thinking.push_str(fragment),
    ...
    pub(crate) fn add_signature(&mut self, fragment: &str) {
        ...
        match self.blocks.last_mut() {
            Some(Block::Thinking { signature, .. }) => signature.push_str(fragment),
            _ => self.blocks.push(Block::Thinking {
                thinking: String::new(),
    ...
    /// Open or continue a tool call, giving back the key later fragments reach
    /// it by. Pass the number the endpoint gave the call, or `None` when it
    /// numbers none.
    pub(crate) fn open_tool_call(
        ...
        let key = named.map_or_else(|| self.key_for(id), ToolCallKey::Named);
    ...
    fn key_for(&self, id: &str) -> ToolCallKey {
        if id.is_empty() {
            ...
            return in_flight.unwrap_or(ToolCallKey::Unnamed(0));
        }
        let held = self
            .blocks
            .iter()
        ...
        held.unwrap_or_else(|| ToolCallKey::Unnamed(self.tool_call_count()))
    }
...
mod tests {
    ...
    fn malformed_arguments_stay_verbatim() {
        // Blanking to `{}` would drop the call with no trace, and the schema
        // decode would report a fabricated problem rather than the real one.
    ...
    fn a_high_number_from_the_endpoint_adds_only_the_one_call() {
        // The number reaches the reply from the endpoint, so it must route a
