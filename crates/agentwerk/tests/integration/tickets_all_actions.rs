...
//! reading its own ticket and its parent's result, listing and searching the
//! queue, then creating and editing a ticket. The role names intents, never
//! actions, so the tool's own description is what has to map each intent onto
//! the right action. The parent's result comes from a real handover, the only
//! way a finished ticket with a result exists.
...
use agentwerk::{Agent, EventKind, Ticket, TicketQueue};

const ACTIONS: [&str; 6] = ["ticket", "result", "list", "search", "create", "edit"];

/// Sits unlabeled and unclaimed: both agents carry a label, so neither handles
/// it. It exists to give `list` and `search` something to find.
        ...
        } = &e.kind
        {
            if tool_name == "tickets" {
                if let Some(action) = input["action"].as_str() {
                    recorder.lock().unwrap().insert(action.to_string());
                }
            }
        }
    });
        ...
        Agent::new()
            ...
            .role(
                "{context}\n\n\
                 ...
                 4. Without reading the queue again, match the phrase `sealed \
                 archive room` against ticket bodies to find the one ticket \
                 carrying it.\n\
