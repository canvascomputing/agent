...
- Finish the ticket by calling `finish` with `result` holding `idx` and `partial_sum`: `finish({"result": {"idx": IDX, "partial_sum": N}})`, copying `idx` verbatim from the task and using the integer the tool printed for `N`.
