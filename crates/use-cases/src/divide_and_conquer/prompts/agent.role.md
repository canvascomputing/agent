...
- Finish the ticket by calling `finish` with `idx` and `partial_sum` as its top-level arguments: `finish({"idx": IDX, "partial_sum": N})`, copying `idx` verbatim from the task and using the integer the tool printed for `N`.
