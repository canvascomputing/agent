...
- MUST first call `read_tickets` with `action="get"` and NO `key`. This returns YOUR current ticket including its `parent:` line. Note the parent value.
- MUST then call `read_tickets` with `action="get"` and `key` set to the parent value (e.g. `"TICKET-1"`, NOT the literal string `"TICKET-N"`). This returns researcher_1's findings.
