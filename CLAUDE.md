...
agentwerk is a minimal Rust crate that gives any application agentic capabilities, plus Python bindings for the same API. It includes only what an agentic workflow needs. Read the convention file for your task, change the code, and leave every document true to the result. The next agent reads those documents instead of your diff.
